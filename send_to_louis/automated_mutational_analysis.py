...
2) run single pack and minimization with nstruct=1000, dumping the structures into base_structs
...
6) run two pack and minimizations on the mutant with nstruct=1000, dumping the structures into base_structs
'''
...
parser.add_argument("pdb_filename", type=str, help="the filename of the native PDB structure")
...
# base_structs, lowest_E_structs, and mut_structs
...
from make_all_mutations import make_all_mutations
...
# make the directory for the PDB in the base_structs_dir
...
# make nstruct of the native doing one pack and minimization to get a standard low E structure
...
make_all_mutations( lowest_E_native_filename, input_args.mutant_list_file, mut_structs_dir )
...
# for each filename, make nstruct decoys by pack and minimization
for mutation in short_mut_filenames:
    ...
    # create and use the PyJobDistributor
    jd = PyJobDistributor( mut_decoy_pdb_name, input_args.nstruct, sf )
    jd.native_pose = native_pose
