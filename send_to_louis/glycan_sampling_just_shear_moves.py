...
metrics_dump_dir = working_dir + "shear_%s_dir" %str( input_args.num_shear_move_trials )
try:
...
if not os.path.isdir( structure_dir ):
    try:
        ...
        pass
working_pose_decoy_name = structure_dir + '/' + working_pdb_name + "_glycosylated_then_just_%s_shear_moves" %input_args.num_shear_move_trials
...
info_file_details.append( "Number of shear move trials:\t%s ( 1 trial = 5 moves )\n" %str( input_args.num_shear_move_trials ) )
...
print "Running ShearMover PyJobDistributor..."
...
while not jd.job_complete:
    ...
    sh = ShearMover( movemap_in = shear_mm, temperature_in = kT, nmoves_in = 5 )
    sh.scorefxn( main_sf )
