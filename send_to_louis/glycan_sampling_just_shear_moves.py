...
metrics_dump_dir = working_dir + "shear_%s_dir" %str( input_args.num_shear_moves )
try:
...
if not os.path.isdir( structure_dir ):
    try:
        ...
        pass
working_pose_decoy_name = structure_dir + '/' + working_pdb_name + "_glycosylated_then_just_%s_shear_moves" %input_args.num_shear_moves
...
while not jd.job_complete:
    ...
    num_sm_accept = 0
    num_mc_checks = 0
    for ii in range( 1, input_args.num_shear_moves + 1 ):
        # if score ramping is desired
        if input_args.ramp_sf:
