...
parser.add_argument("num_move_trials", type=int, help="how many trials of 5 SmallMoves/ShearMoves do you want to make within the Fc glycan?")
...
metrics_dump_dir = working_dir + "smsh_%s_dir" %str( input_args.num_move_trials )
try:
...
from rosetta import SmallMover, ShearMover, MinMover
...
if not os.path.isdir( structure_dir ):
    try:
        ...
        pass
working_pose_decoy_name = structure_dir + '/' + working_pdb_name + "_glycosylated_then_just_%s_SmSh_moves" %input_args.num_move_trials
...
info_file_details.append( "Number of Small and ShearMove trials:\t%s ( 1 trial = 5 moves )\n" %str( input_args.num_move_trials ) )
...
print "Running Small and ShearMover PyJobDistributor..."
...
while not jd.job_complete:
    ...
    ##############################
    #### Small and ShearMover ####
    ##############################

    ## use the Small and ShearMover find a local sugar minima
    # make a MoveMap for these Fc sugars
    smsh_mm = make_movemap_for_range( Fc_sugar_nums_except_core_GlcNAc, 
                                      allow_bb_movement = True, 
                                      allow_chi_movement = True )

    # add in the branch points myself ( does not include the two ASN residues )
    for branch_point in Fc_glycan_branch_point_nums:
        smsh_mm.set_branches( branch_point, True )

    # make an appropriate MonteCarlo object
    mc = MonteCarlo( testing_pose, main_sf, kT )

    # make an appropriate SmallMover and ShearMover
    sm = SmallMover( movemap_in = smsh_mm, temperature_in = kT, nmoves_in = 5 )
    sm.scorefxn( main_sf )
    sh = ShearMover( movemap_in = smsh_mm, temperature_in = kT, nmoves_in = 5 )
    sh.scorefxn( main_sf )
    ...
    # run the SmallMover and ShearMover randomly 10-100 times using a MonteCarlo object to accept or reject the move
    num_smsh_accept = 0
    num_mc_checks = 0
    for ii in range( 1, input_args.num_move_trials + 1 ):
        # if score ramping is desired
        if input_args.ramp_sf:
        ...
        # randomly either apply the SmallMover or ShearMover
        if random.randint( 1, 2 ) == 1: # 50% chance of occuring
        ...
        num_mc_checks += 1
        mc_acceptance = round( ( float( num_smsh_accept ) / float( num_mc_checks ) * 100 ), 3 )
        if input_args.verbose:
            print
            print "Moves made so far:", num_mc_checks, 
            print "  Moves accepted:", num_smsh_accept, 
