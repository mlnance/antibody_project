...
'''
SAMPLE INPUT
run glycan_sampling_just_sugar_shear_moves_on_native.py project_structs/lowest_E_double_pack_and_min_only_native_crystal_struct_3ay4_Fc_FcgRIII.pdb project_glyco_files/3ay4_Fc_Glycan.iupac project_utility_files/ ~/pyrosetta_dir/test_pdb_dir/ 1 3 3
'''
...
#parser.add_argument("--just_chain_A", action="store_true", help="do you want the SugarShearMover (and any reset) to act on the chain A glycan only?")
...
parser.add_argument("--angle_multiplier", type=float, default=1.0, help="by how much do you want to increase the angle_max for the SugarShearMover? Default is 1.0 (standard value)" )
...
# check that num_sugar_shear_move_trials is 10 or more if ramp_sf is set to True
if input_args.ramp_sf == True and input_args.num_sugar_shear_move_trials < 10:
    print "\nIf you are ramping the ScoreFunction then you need to do 10 or more sugar_shear_move_trials for the math to work. Exiting."
    sys.exit()
...
metrics_dump_dir = working_dir + "sugar_shear_dir_%s" %str( input_args.num_sugar_shear_move_trials )
try:
...
if not os.path.isdir( structure_dir ):
    try:
        ...
        pass
working_pose_decoy_name = structure_dir + '/' + native_pdb_name + "_just_%s_sugar_shear_moves" %input_args.num_sugar_shear_move_trials
...
print "Running SugarShearMover PyJobDistributor..."
...
while not jd.job_complete:
    ...
    ## use the SugarShearMover find a local sugar minima
    # make an appropriate MonteCarlo object
    mc = MonteCarlo( testing_pose, main_sf, kT )

    # make a ShearMover as to get the angle_max value
    sh = ShearMover()
    ...
    # run the SugarShearMover a range of times using a MonteCarlo object to accept or reject the move
    num_ssh_accept = 0
    num_mc_checks = 0
    mc_acceptance = None
    for ii in range( 1, input_args.num_sugar_shear_move_trials + 1 ):
        ...
        if input_args.ramp_sf:
            ...
            main_sf = ramp_score_weight( main_sf, 
                                         ...
                                         FA_ATR_ORIG, 
                                         ii - 1, 
                                         input_args.num_sugar_shear_move_trials )
            main_sf = ramp_score_weight( main_sf, 
                                         "fa_rep", 
                                         FA_REP_ORIG, 
                                         ii - 1, 
        ...
        if ii % 3 == 0 or ii == input_args.num_sugar_shear_move_trials:
            if input_args.verbose:
